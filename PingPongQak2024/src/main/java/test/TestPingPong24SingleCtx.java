	...
	Thread th = new Thread(){
		public void run(){
			try {
				...
				Process p = Runtime.getRuntime().exec("../iss2024Run/pingpong24-1.0/bin/pingpong24");
				showOutput(p,ColorsOut.BLACK);
			} catch ( Exception e) {
				CommUtils.outred("TestPingPong24SingleCtx activate ERROR " + e.getMessage());
			}
...
/*
 * Connect with the referee
 * Request info to the referee
 * Assertion on the answer of the referee
 */
	@Test
		...
		IApplMessage req  = CommUtils.buildRequest( "tester", "info", "info(X)", "referee");
 		try {
			...
			while( connSupport == null ) {
 				connSupport = ConnectionFactory.createClientSupport(ProtocolType.tcp, "localhost", "8020");
